import { SHAPES } from 'bloub-react';
...
import {
  ...
  sansCommentaires,
  viewBoxExport,
} from '@/bloub/ui/export';

/** Aligné sur `bot/repere` du package (non exporté). */
const RAYON = 100;

/** Demi-cote du viewBox affiche a l'ecran, cf. le `VB` de BloubBot. */
const VB_ECRAN = 158;
...
describe('cadre d export', () => {
  ...
  it('se cadre sur la forme la plus etalee et non sur le cercle', () => {
    ...
    expect(RAYON_MAX).toBe(
      Math.max(...SHAPES.map((f) => Math.max(...f.radii)))
    );
  });
...
describe('catalogue des exports', () => {
  ...
  it('copie le bitmap en image et le vectoriel en texte', () => {
    for (const action of ACTIONS.filter((a) => a.mode === 'copieImage')) {
      expect(action.extension).toBe('png');
    }
    for (const action of ACTIONS.filter((a) => a.mode === 'copieTexte')) {
      expect(action.extension).toBe('svg');
    }
  });
