...
import { type Block, BloubBot, type BloubBotHandle } from 'bloub-react';
import {
  gifAnime,
  gifIndexe,
  indexe,
...
export async function sequenceDuBot<T>(
  ...
  const rendre = (frozenAt: number) => {
    // Off-screen capture must commit before we read the SVG DOM.
    // eslint-disable-next-line @eslint-react/dom-no-flush-sync -- sync paint required for serialization
    flushSync(() => {
      root.render(
        createElement(BloubBot, {
          ...reglages,
          size: taille,
          frozenAt,
          ...(paper ? { paper } : {}),
...
export async function ouvreCycle(
  ...
  let bot: BloubBotHandle | null = null;
  const root = createRoot(hote);
  ...
  flushSync(() => {
    root.render(
      createElement(BloubBot, {
        ...
        frozenAt: 0,
        ref: (handle: BloubBotHandle | null) => {
          bot = handle;
        },
        ...(paper ? { paper } : {}),
      })
...
function matricesDesYeux(svg: SVGSVGElement) {
  return [...svg.querySelectorAll('mask [transform]')].map(
