...
import { blocksWith, nextCycleId, uniqueName } from '../ui/cycle-edit';
...
export function Timeline({
  ...
  function onNamed(name: string) {
    ...
    if (demande.mode === 'create') {
      ...
      onCyclesChange([...cycles, neuf]);
      select(neuf.id);
      return;
    }
    const autres = cycles.filter((c) => c.id !== demande.id);
    const unique = uniqueName(name, autres);
    onCyclesChange(
      cycles.map((c) => (c.id === demande.id ? { ...c, name: unique } : c))
    );
  }

  /** Suppression d'un montage : jamais sans confirmation, c'est irreversible. */
  function askRemove(id: string) {
    setRemoving(cycles.find((c) => c.id === id) ?? null);
    setConfirmOpen(true);
  }

  function onRemove() {
    const cible = removing;
    setRemoving(null);
    if (!cible) return;
    const reste = cycles.filter((c) => c.id !== cible.id);
    onCyclesChange(reste);
    if (cible.id === activeId) select(reste[0]!.id);
  }
    ...
    */
    <div className="fixed inset-x-0 bottom-0 z-30 h-[var(--timeline)] px-6 pt-3 pb-5 max-lg:border-t max-lg:border-[var(--line)] max-lg:bg-[var(--paper)] max-lg:px-5 lg:right-[24.5rem] lg:left-[4.5rem]">
      ...
      <div className="absolute -top-5 left-1/2 flex -translate-x-1/2 items-center gap-3">
        <span className="text-sm font-medium tabular-nums max-lg:hidden">
          {mmss(at)}
        </span>
        <button
          type="button"
        ...
        <span className="text-sm text-[var(--muted)] tabular-nums max-lg:hidden">
          {mmss(total)}
        </span>
      </div>
      ...
      <div className="flex h-full flex-col gap-2 select-none">
        ...
        */}
        ...
        <TimelineTrack
          ...
          onBlocksChange={(b: Block[]) => edit({ blocks: b })}
          onAdd={(s: StateId) => edit({ blocks: blocksWith(blocks, s) })}
          onSeek={onSeek}
        />
        ...
        <div className="flex shrink-0 items-center justify-end gap-4 max-sm:gap-2">
          ...
          <p className="text-xs text-[var(--muted)] tabular-nums">
            <span className="text-[var(--ink)]">{mmss(at)}</span> /{' '}
            {mmss(total)}
          </p>
          ...
          <span className="group relative flex">
            ...
            <span
              className="pointer-events-none absolute bottom-full left-1/2 mb-2 -translate-x-1/2 translate-y-1 rounded-lg bg-[var(--ink)] px-2.5 py-1.5 text-xs whitespace-nowrap text-[var(--paper)] opacity-0 transition group-focus-within:translate-y-0 group-focus-within:opacity-100 group-hover:translate-y-0 group-hover:opacity-100"
              role="tooltip"
            >
              {t('timeline.preview')}
            </span>
      ...
      <ConfirmDialog
        ...
        title={t('dialog.removeTitle', {
          name: removing ? nomDeCycle(removing) : '',
        })}
