...
export function ZoomSlider({ zoom, onZoomChange, min, max }: ZoomSliderProps) {
  const percent = pourcentage(zoom);
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span
        className="h-1 w-1 shrink-0 rounded-full bg-[var(--muted)]"
        aria-hidden="true"
      />
      <input
        type="range"
      ...
      />
      <span
        className="h-2 w-2 shrink-0 rounded-full bg-[var(--muted)]"
        aria-hidden="true"
      />
      ...
      <span className="w-11 text-left text-xs text-[var(--muted)] tabular-nums">
        {percent}
      </span>
    </div>
  );
