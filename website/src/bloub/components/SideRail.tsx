...
export function SideRail({
   ...
   */
  const [muted, setMuted] = useState<ViewId | null>(null);
  ...
  return (
    ...
    */
    <nav
      className={`fixed top-3 left-1/2 z-20 -translate-x-1/2 rounded-2xl border border-[var(--line)] bg-white/85 p-1.5 shadow-sm backdrop-blur lg:top-1/2 lg:left-4 lg:translate-x-0 lg:-translate-y-1/2 ${className ?? ''}`}
      ...
      <ul className="flex gap-1 lg:flex-col">
        {ITEMS.map((item) => (
          ...
          >
            <button
              type="button"
              className={`peer flex h-10 w-10 cursor-pointer items-center justify-center rounded-xl transition ${
                view === item.id
              ...
              onPointerDown={() => setMuted(item.id)}
            ...
            */}
            <span
              className={`pointer-events-none absolute top-full left-1/2 z-10 mt-2 -translate-x-1/2 rounded-lg bg-[var(--ink)] px-2.5 py-1.5 text-xs whitespace-nowrap text-[var(--paper)] opacity-0 transition group-hover:opacity-100 peer-focus-visible:opacity-100 lg:top-1/2 lg:left-full lg:mt-0 lg:ml-2 lg:translate-x-1 lg:-translate-y-1/2 lg:group-hover:translate-x-0 lg:peer-focus-visible:translate-x-0 ${
                muted === item.id ? 'opacity-0! lg:translate-x-1!' : ''
              }`}
              role="tooltip"
            >
              {item.label}
